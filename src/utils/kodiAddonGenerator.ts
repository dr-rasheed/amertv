...
export function generatePythonMainScript(config: KodiAddonConfig, databaseItems: UnifiedMediaItem[]): string {
  const repoRawUrl = `${config.repoUrl.replace('https://github.com/', 'https://raw.githubusercontent.com/').replace('https://', 'https://')}/media_database.json`;
...
REMOTE_DB_URL = "${config.repoUrl}/media_database.json"
